...
while IFS= read -r string; do
	if ! grep -q "\"$string\"" hestiacp.pot; then
		echo -e "\n#: ../../bin/h-list-sys-hestia-updates:$(grep -n "$string" ../../bin/h-list-sys-hestia-updates | cut -d: -f1)\nmsgid \"$string\"\nmsgstr \"\"" >> hestiacp.pot
	fi
done < <(awk -F'DESCR=' '/data=".+ DESCR=[^"]/ {print $2}' ../../bin/h-list-sys-hestia-updates | cut -d\' -f2)

# Scan the description string for list server page
while IFS= read -r string; do
	if ! grep -q "\"$string\"" hestiacp.pot; then
		echo -e "\n#: ../../bin/h-list-sys-services:$(grep -n "$string" ../../bin/h-list-sys-services | cut -d: -f1)\nmsgid \"$string\"\nmsgstr \"\"" >> hestiacp.pot
	fi
done < <(awk -F'SYSTEM=' '/data=".+ SYSTEM=[^"]/ {print $2}' ../../bin/h-list-sys-services | cut -d\' -f2)
