...
is_valid_extension() {
	...
	if [ ! -e "/etc/hestia/extensions/public_suffix_list.dat" ]; then
		mkdir -p "/etc/hestia/extensions/"
		chmod 750 "/etc/hestia/extensions/"
		if /usr/bin/wget --tries=3 --timeout=15 --read-timeout=15 --waitretry=3 --no-dns-cache --quiet -O "/etc/hestia/extensions/public_suffix_list.dat.tmp" "$psl"; then
			mv "/etc/hestia/extensions/public_suffix_list.dat.tmp" "/etc/hestia/extensions/public_suffix_list.dat"
		else
			rm -f "/etc/hestia/extensions/public_suffix_list.dat.tmp"
		fi
	elif find "/etc/hestia/extensions/public_suffix_list.dat" -mtime +7 2> /dev/null | grep -q .; then
		mv "/etc/hestia/extensions/public_suffix_list.dat" "/etc/hestia/extensions/public_suffix_list.dat.save"
		if /usr/bin/wget --tries=3 --timeout=15 --read-timeout=15 --waitretry=3 --no-dns-cache --quiet -O "/etc/hestia/extensions/public_suffix_list.dat.tmp" "$psl"; then
			mv "/etc/hestia/extensions/public_suffix_list.dat.tmp" "/etc/hestia/extensions/public_suffix_list.dat"
			rm -f "/etc/hestia/extensions/public_suffix_list.dat.save"
		else
			rm -f "/etc/hestia/extensions/public_suffix_list.dat.tmp"
			mv "/etc/hestia/extensions/public_suffix_list.dat.save" "/etc/hestia/extensions/public_suffix_list.dat"
		fi
	fi
	if [ ! -e "/etc/hestia/extensions/public_suffix_list.dat" ]; then
	...
	exten=$(grep -Fx "$extension" "/etc/hestia/extensions/public_suffix_list.dat")
}
