...
fn_ask_pre_questions() {
    ...
    if [ "$AUTO_MODE" = true ]; then
        # Unattended (-a): take the same defaults the prompts would propose, no
        # questions. The admin password is generated by h-install-hestia either
        # way (never prompted), so this makes the whole run non-interactive.
