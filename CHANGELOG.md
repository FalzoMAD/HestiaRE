...
- **Two firewall smoke guards** (#481) - `h-check-sys-smoke` now checks the firewall *datapath*, not just
  ...
  matches on has something that recreates it at boot. The second reads the saved file on purpose: the
  kernel refuses `--match-set` for a set that does not exist, so a dangling reference cannot exist in the
  live ruleset and looking for one there would always pass. The reachable failure is ordering -
  ...
  dash-prefixed and its failure ignored. Both guards were negative-tested (forced ACCEPT policy; an
  injected reference to an unprovisioned set) and are green across the fleet.
